...
- Jetpack Compose (BOM 2025.12、Material3、Foundation、Runtime LiveData) + ViewBinding 併用
- Hilt 2.57 + KSP（DI）、WorkManager（背景監視）
- Coil 3.3 (画像/動画/GIF/SVG キャッシュ) + OkHttp 5.3 / Jsoup 1.21 (通信・HTML 解析)
...
- Android 端末 / エミュレータ: Android 7.0 (API 24) 以降、ターゲット API 36
- JDK 17、Gradle 8.13 系
...
- 動画編集は `videoeditor` パッケージに MVI + Clean Architecture で独立実装されており、`EditorActivity` がエントリポイントです。
